...
def random_component_by_score(component_type, component_map):
    ...
    # From redis, get hash to get sum of key name
    values = client.hgetall(f"{component_type}_score_values")
    
    # From redis get hash with total counts/times each has been voted for.
    counts = client.hgetall(f"{component_type}_score_counts")
    
    # Get all valid component names
    names = component_map.keys() 

    # loop over valid names to calc average score for each
    avg_scores = {}
    for name in names:
        score = int(values.get(name, 1))
        count = int(counts.get(name, 1))
        avg = score/count
        # first vote could be downvote, set atleast to 0.1 so it can be selected
        # in the future
...
def score_conversation(
    ...
    # Store value at this hash/key in redis
